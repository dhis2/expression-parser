...
internal class HasUserRoleTest {
    ...
    fun testHasUserRole_Null() {
        assertFalse(evaluate("d2:hasUserRole(null)",mapOf(SupplementaryKey.Fixed(FixedKey.USER_ROLES) to listOf("admin"))
        ))
    }
    ...
    fun testHasUserRole() {
        assertTrue(
            evaluate(
                "d2:hasUserRole(\"admin\")",
                mapOf(SupplementaryKey.Fixed(FixedKey.USER_ROLES) to listOf("admin"))
            )
        )
        assertFalse(
            evaluate(
                "d2:hasUserRole(\"admin\")",
                mapOf(SupplementaryKey.Fixed(FixedKey.USER_ROLES) to listOf("guest"))
            )
        )
        assertTrue(
            evaluate(
                "d2:hasUserRole(\"admin\")",
                mapOf(SupplementaryKey.Fixed(FixedKey.USER_ROLES) to listOf("foo", "admin"))
            )
        )
    }
