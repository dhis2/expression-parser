...
internal class CountIfValue : AbstractVariableBasedTest() {
    ...
    fun testCountIfValue_Null() {
        val values = mapOf("v1" to VariableValue(ValueType.BOOLEAN).copy(candidates = listOf("true")))
        assertEquals(0, evaluate("d2:countIfValue(#{v1}, null)", values))
    }

    @Test
    fun testCountIfValue_NoCandidates() {
        val values = mapOf("v1" to VariableValue(ValueType.BOOLEAN).copy(candidates = listOf()))
        assertEquals(0, evaluate("d2:countIfValue(#{v1}, true)", values))
    }

    @Test
    fun testCountIfValue_Number() {
        val values = mapOf("v1" to VariableValue(ValueType.NUMBER).copy(candidates = listOf("1", "2", "1")))
    ...
    fun testCountIfValue_Boolean() {
        val values = mapOf("v1" to VariableValue(ValueType.BOOLEAN).copy(candidates = listOf("true", "false", "true")))
    ...
    fun testCountIfValue_String() {
        val values = mapOf("v1" to VariableValue(ValueType.STRING).copy(candidates = listOf("a", "b", "c", "a")))
