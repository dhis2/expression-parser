...
internal class CountIfZeroPosTest : AbstractVariableBasedTest() {
    ...
    fun testCountIfZeroPos_Zero() {
        val values = mapOf("v1" to VariableValue(ValueType.NUMBER).copy(candidates = listOf("0", "0.0")))
        assertEquals(2, evaluate("d2:countIfZeroPos(#{v1})", values))
    }

    @Test
    fun testCountIfZeroPos_Positive() {
        val values = mapOf("v1" to VariableValue(ValueType.NUMBER).copy(candidates = listOf("1", "2", "0.0")))
        assertEquals(3, evaluate("d2:countIfZeroPos(#{v1})", values))
    }

    @Test
    fun testCountIfZeroPos_Negative() {
        val values = mapOf("v1" to VariableValue(ValueType.NUMBER).copy(candidates = listOf("-2", "-1", "-0.2")))
        assertEquals(0, evaluate("d2:countIfZeroPos(#{v1})", values))
    }

    @Test
    fun testCountIfZeroPos_Mixed() {
        val values = mapOf("v1" to VariableValue(ValueType.NUMBER).copy(candidates = listOf("1", "2", "-1", "0.2", "0.0")))
