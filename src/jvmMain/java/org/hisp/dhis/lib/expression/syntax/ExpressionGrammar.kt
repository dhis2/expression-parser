...
interface ExpressionGrammar {
    ...
    Block expressions
     */
        fun mod(modifier: DataItemModifier, vararg args: Fragment): Fragment {
            val name = modifier.name
            return block(NodeType.MODIFIER, name, '(', ',', ')', *args).named(name)
        }

        fun fn(function: NamedFunction, vararg args: Fragment): Fragment {
        ...
        fun block(
            ...
            return Fragment { expr: Expr, ctx: ParseContext ->
                ...
                var i = 0
                while (i < args.size || args.size > 0 && args[args.size - 1].isVarargs()) {
                    expr.skipWS()
     ...
     */
        val STRING: Terminal = Terminal{ NodeType.STRING }
        val INTEGER: Terminal = Terminal { NodeType.INTEGER }
        val UID: Terminal = Terminal { NodeType.UID }
        val IDENTIFIER: Terminal = Terminal { NodeType.IDENTIFIER }
        val DATE: Terminal = Terminal { NodeType.DATE }

        /*
    Essential composed building blocks
     */
        val expr: Fragment = Fragment {expr, ctx ->  Expr.expr(expr, ctx)}
        val dataItem: Fragment = Fragment {expr, ctx -> Expr.dataItem(expr, ctx)}
        val dataItemHash: Fragment = Fragment {expr, ctx -> Expr.dataItemHash(expr, ctx)}
        val dataItemA: Fragment = Fragment {expr, ctx -> Expr.dataItemA(expr, ctx)}

        /*
    Production Rules
     */
        val STAGE_OFFSET = mod(DataItemModifier.stageOffset, INTEGER)
        val MAX_DATE = mod(DataItemModifier.maxDate, DATE)
        val MIN_DATE = mod(DataItemModifier.minDate, DATE)
        val AGGREGATION_TYPE = mod(DataItemModifier.aggregationType, IDENTIFIER.`as`(Node.Factory.new(::AggregationTypeNode)))
        val PERIOD_OFFSET = mod(DataItemModifier.periodOffset, INTEGER)
        val YEAR_TO_DATE = mod(DataItemModifier.yearToDate)
        val SUB_EXPRESSION = fn(NamedFunction.subExpression, expr)

        val CommonFunctions = listOf( // (alphabetical)
        ...
        val ValidationRuleFunctions = listOf(
        ...
        val CommonAggregationFunctions = listOf(
        ...
        val PredictorAggregationFunctions = listOf( // (alphabetical)
        ...
        val CommonD2Functions = listOf( // (alphabetical)
        ...
        val RuleEngineD2Functions = listOf(
        ...
        val ProgramIndicatorD2Functions = listOf(
        ...
        val CommonConstants = listOf(
            constant(NodeType.NULL, "null"),
            constant(NodeType.BOOLEAN, "true"),
            constant(NodeType.BOOLEAN, "false")
        )
        val HASH_BRACE = dataItemHash.named(DataItemType.DATA_ELEMENT.symbol)
        val A_BRACE = dataItemA.named(DataItemType.ATTRIBUTE.symbol)
        val C_BRACE = item(DataItemType.CONSTANT, UID)
        val D_BRACE = item(DataItemType.PROGRAM_DATA_ELEMENT, UID, UID)
        val I_BRACE = item(DataItemType.PROGRAM_INDICATOR, UID)
        val R_BRACE = item(DataItemType.REPORTING_RATE, UID, IDENTIFIER.`as`(Node.Factory.new(::ReportingRateTypeNode)))
        val OUG_BRACE = item(DataItemType.ORG_UNIT_GROUP, UID)
        val N_BRACE = item(DataItemType.INDICATOR, UID)
        val V_BRACE = variable(DataItemType.PROGRAM_VARIABLE, IDENTIFIER.`as`(Node.Factory.new(::ProgramVariableNode)))
        val CommonDataItems = listOf(HASH_BRACE, A_BRACE, C_BRACE, D_BRACE, I_BRACE, R_BRACE, OUG_BRACE)
