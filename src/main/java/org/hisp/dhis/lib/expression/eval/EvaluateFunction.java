...
class EvaluateFunction implements NodeInterpreter<Object> {
    ...
    public Object evalBinaryOperator(Node<BinaryOperator> operator) {
        ...
        switch (operator.getValue()) {
            case EQ:
                return evalBinaryOperator(BinaryOperator::equal, operator, this::evalToMixed);
            case NEQ:
                return evalBinaryOperator(BinaryOperator::notEqual, operator, this::evalToMixed);
            case AND:
                return evalBinaryOperator(BinaryOperator::and, operator, this::evalToBoolean);
            case OR:
                return evalBinaryOperator(BinaryOperator::or, operator, this::evalToBoolean);
            case LT:
                return evalBinaryOperator(BinaryOperator::lessThan, operator, this::evalToMixed);
            case LE:
                return evalBinaryOperator(BinaryOperator::lessThanOrEqual, operator, this::evalToMixed);
            case GT:
                return evalBinaryOperator(BinaryOperator::greaterThan, operator, this::evalToMixed);
            case GE:
                return evalBinaryOperator(BinaryOperator::greaterThanOrEqual, operator, this::evalToMixed);
            case ADD:
                return evalBinaryOperator(BinaryOperator::add, operator, this::evalToNumber);
            case SUB:
                return evalBinaryOperator(BinaryOperator::subtract, operator, this::evalToNumber);
            case MUL:
                return evalBinaryOperator(BinaryOperator::multiply, operator, this::evalToNumber);
            case DIV:
                return evalBinaryOperator(BinaryOperator::divide, operator, this::evalToNumber);
            case MOD:
                return evalBinaryOperator(BinaryOperator::modulo, operator, this::evalToNumber);
            case EXP:
                return evalBinaryOperator(BinaryOperator::exp, operator, this::evalToNumber);
            default:
    ...
    public Object evalUnaryOperator(Node<UnaryOperator> operator) {
        ...
        switch (operator.getValue()) {
            case NOT:
                return !evalToBoolean(operand);
            case PLUS:
                return evalToNumber(operand);
            case MINUS:
                return UnaryOperator.negate(evalToNumber(operand));
            default:
    ...
    public Object evalFunction(Node<NamedFunction> fn) {
        ...
        switch (fnInfo) {
            // common functions
            case firstNonNull:
                return functions.firstNonNull(evalToMixed(fn.children()));
            case greatest:
                return functions.greatest(evalToNumbers(fn.children()));
            case ifThenElse:
                return functions.ifThenElse(evalToBoolean(fn.child(0)), evalToMixed(fn.child(1)), evalToMixed(fn.child(2)));
            case isNotNull:
                return functions.isNotNull(evalToMixed(fn.child(0)));
            case isNull:
                return functions.isNull(evalToMixed(fn.child(0)));
            case least:
                return functions.least(evalToNumbers(fn.children()));
            case log:
                return fn.size() == 1
                        ? functions.log(evalToNumber(fn.child(0)))
                        : functions.log(evalToNumber(fn.child(0))) / functions.log(evalToNumber(fn.child(1)));
            case log10:
                return functions.log10(evalToNumber(fn.child(0)));
            case removeZeros:
                return functions.removeZeros(evalToNumber(fn.child(0)));

            // d2 functions
            case d2_addDays:
                return functions.d2_addDays(evalToDate(fn.child(0)), evalToNumber(fn.child(1)));
            case d2_ceil:
                return functions.d2_ceil(evalToNumber(fn.child(0)));
            case d2_concatenate:
                return functions.d2_concatenate(evalToStrings(fn.children()));
            case d2_count:
                return functions.d2_count(evalToVar(fn.child(0)));
            case d2_countIfValue:
                return functions.d2_countIfValue(evalToVar(fn.child(0)), evalToMixed(fn.child(1)));
            case d2_countIfZeroPos:
                return functions.d2_countIfZeroPos(evalToVar(fn.child(0)));
            case d2_daysBetween:
                return functions.d2_daysBetween(evalToDate(fn.child(0)), evalToDate(fn.child(1)));
            case d2_extractDataMatrixValue:
                return functions.d2_extractDataMatrixValue(evalToString(fn.child(0)), evalToString(fn.child(1)));
            case d2_floor:
                return functions.d2_floor(evalToNumber(fn.child(0)));
            case d2_hasUserRole:
                return functions.d2_hasUserRole(evalToString(fn.child(0)), data.getSupplementaryValues().get("USER"));
            case d2_hasValue:
                return functions.d2_hasValue(evalToVar(fn.child(0)));
            case d2_inOrgUnitGroup:
                return functions.d2_inOrgUnitGroup(evalToString(fn.child(0)), data.getProgramRuleVariableValues().get("org_unit"), data.getSupplementaryValues());
            case d2_lastEventDate:
                return functions.d2_lastEventDate(data.getProgramRuleVariableValues().get(evalToString(fn.child(0))));
            case d2_left:
                return functions.d2_left(evalToString(fn.child(0)), evalToInteger(fn.child(1)));
            case d2_length:
                return functions.d2_length(evalToString(fn.child(0)));
            case d2_maxValue:
                return functions.d2_maxValue(evalToVar(fn.child(0)));
            case d2_minutesBetween:
                return functions.d2_minutesBetween(evalToDate(fn.child(0)), evalToDate(fn.child(1)));
            case d2_minValue:
                return functions.d2_minValue(evalToVar(fn.child(0)));
            case d2_modulus:
                return functions.d2_modulus(evalToNumber(fn.child(0)), evalToNumber(fn.child(1)));
            case d2_monthsBetween:
                return functions.d2_monthsBetween(evalToDate(fn.child(0)), evalToDate(fn.child(1)));
            case d2_oizp:
                return functions.d2_oizp(evalToNumber(fn.child(0)));
            case d2_right:
                return functions.d2_right(evalToString(fn.child(0)), evalToInteger(fn.child(1)));
            case d2_round:
                return functions.d2_round(evalToNumber(fn.child(0)), fn.size() <= 1 ? Integer.valueOf(0) : evalToInteger(fn.child(1)));
            case d2_split:
                return functions.d2_split(evalToString(fn.child(0)), evalToString(fn.child(1)), evalToInteger(fn.child(2)));
            case d2_substring:
                return functions.d2_substring(evalToString(fn.child(0)), evalToInteger(fn.child(1)), evalToInteger(fn.child(2)));
            case d2_validatePattern:
                return functions.d2_validatePattern(evalToString(fn.child(0)), evalToString(fn.child(1)));
            case d2_weeksBetween:
                return functions.d2_weeksBetween(evalToDate(fn.child(0)), evalToDate(fn.child(1)));
            case d2_yearsBetween:
                return functions.d2_yearsBetween(evalToDate(fn.child(0)), evalToDate(fn.child(1)));
            case d2_zing:
                return functions.d2_zing(evalToNumber(fn.child(0)));
            case d2_zpvc:
                return functions.d2_zpvc(evalToNumbers(fn.children()));
            case d2_zScoreHFA:
                return functions.d2_zScoreHFA(evalToNumber(fn.child(0)), evalToNumber(fn.child(1)), evalToString(fn.child(2)));
            case d2_zScoreWFA:
                return functions.d2_zScoreWFA(evalToNumber(fn.child(0)), evalToNumber(fn.child(1)), evalToString(fn.child(2)));
            case d2_zScoreWFH:
                return functions.d2_zScoreWFH(evalToNumber(fn.child(0)), evalToNumber(fn.child(1)), evalToString(fn.child(2)));

            // "not implemented yet"
            default:
                return functions.unsupported(fnInfo.getName());
        }
    }
    ...
    private Double evalAggFunction(Node<NamedFunction> fn) {
        ...
        switch (fn.getValue()) {
            ...
            case count:
                return functions.count(values);
            case max:
                return functions.max(values);
            case median:
                return functions.median(values);
            case min:
                return functions.min(values);
            case percentileCont:
                return functions.percentileCont(values, evalToNumber(fn.child(1)));
            case stddev:
                return functions.stddev(values);
            case stddevPop:
                return functions.stddevPop(values);
            case stddevSamp:
                return functions.stddevSamp(values);
            case sum:
                return functions.sum(values);
            case variance:
                return functions.variance(values);
            default:
                throw new UnsupportedOperationException();
        }
    }
